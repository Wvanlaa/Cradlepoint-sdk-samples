"""
    ...
    the LAN/WAN/WWAN/Modem/IPSec VPN / IP Verify status

    Description is updated every 5 seconds, but only synced to NCM
    if there is a change from the current description
    
    with IPVerify and VPN tunnel the name is added. Best to keep names for IPVerify and VPN Tunnels brief.
        ...
        if wans and ports and mdm_present:
            ...
            if ipverifys:
                ...
                for ipverify in ipverifys:
                    ...
                    testname = cp.get('/status/ipverify/{}/name'.format(ipverify))
                    ports_status += testname
                    if testpass:
                        ports_status += " 🟢 "
                    else:
                        ports_status += " 🔴 "
